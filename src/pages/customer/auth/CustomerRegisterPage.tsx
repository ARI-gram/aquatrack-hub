 ...
 * Multi-step registration flow with customer type selection
 */
...
type Step = 'phone' | 'otp' | 'info' | 'type' | 'bottles' | 'payment' | 'confirm';
...
const CustomerRegisterPage: React.FC = () => {
  ...
  const steps: Step[] = ['phone', 'otp', 'info', 'type', 'bottles', 'payment', 'confirm'];
  ...
  const nextStep = () => {
    ...
    if (nextIndex < steps.length) {
      // Skip bottles step for one-time customers
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    e.preventDefault();
    if (data.otp !== '123456') {
      toast({ title: 'Invalid OTP', description: 'Try 123456 for demo', variant: 'destructive' });
      return;
    }
    nextStep();
  };
  ...
  const calculateTotal = () => {
    ...
    const bottlesCost = data.bottleQuantity * CUSTOMER_PRICING.BOTTLE_PURCHASE_PRICE;
    const depositCost = data.bottleQuantity * CUSTOMER_PRICING.BOTTLE_DEPOSIT;
    return bottlesCost + depositCost;
  };
      ...
      case 'phone':
        return (
          <form onSubmit={handleSendOtp} className="space-y-6">
            ...
            <Button type="submit" className="w-full h-12" variant="ocean" disabled={isLoading}>
        ...
        return (
          <form onSubmit={handleVerifyOtp} className="space-y-6">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-muted-foreground">Enter the code sent to {data.phoneNumber}</p>
            </div>
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-xs text-muted-foreground bg-muted p-2 rounded">💡 Demo: Enter <strong>123456</strong></p>
            </div>
            <div className="flex gap-3">
              <Button type="button" variant="outline" className="flex-1 h-12" onClick={prevStep}>
                <ArrowLeft className="mr-2 h-4 w-4" /> Back
              </Button>
              <Button type="submit" className="flex-1 h-12" variant="ocean" disabled={data.otp.length !== 6}>
                Verify
              </Button>
            </div>
          </form>
          ...
          <form onSubmit={(e) => { e.preventDefault(); nextStep(); }} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="email" type="email" placeholder="ahmed@email.com" value={data.email} onChange={(e) => updateData({ email: e.target.value })} className="h-12" />
              </div>
              <div className="space-y-2">
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      ${CUSTOMER_PRICING.REFILL_PRICE} per refill • Eco-friendly • Track your bottles
                    </p>
                    <p className="text-xs text-muted-foreground mt-2">
                      One-time bottle purchase required (from ${CUSTOMER_PRICING.BOTTLE_PURCHASE_PRICE * 3})
                    </p>
                  </div>
                  {data.customerType === CustomerType.REFILL_CUSTOMER && (
              ...
              <Card
                className={cn(
                  'p-4 cursor-pointer transition-all border-2',
                  data.customerType === CustomerType.ONETIME_CUSTOMER
                    ? 'border-primary bg-primary/5'
                    : 'border-border hover:border-primary/50'
                )}
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      ${CUSTOMER_PRICING.NEW_BOTTLE_PRICE} per bottle • No commitment • New bottle every time
                    </p>
                  </div>
                  {data.customerType === CustomerType.ONETIME_CUSTOMER && (
                    <Check className="h-5 w-5 text-primary" />
            ...
            <p className="text-xs text-center text-muted-foreground">
              💡 You can always switch plans later
            </p>
            <div className="flex gap-3">
              <Button variant="outline" className="flex-1 h-12" onClick={prevStep}>
                <ArrowLeft className="mr-2 h-4 w-4" /> Back
              </Button>
              <Button className="flex-1 h-12" variant="ocean" onClick={nextStep}>
                Continue <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-center gap-4">
              <Button
                variant="outline"
                size="icon"
                onClick={() => updateData({ bottleQuantity: Math.max(3, data.bottleQuantity - 1) })}
                disabled={data.bottleQuantity <= 3}
              >
                -
              </Button>
              ...
              <Button
                variant="outline"
                size="icon"
                onClick={() => updateData({ bottleQuantity: Math.min(20, data.bottleQuantity + 1) })}
                disabled={data.bottleQuantity >= 20}
              >
                +
              </Button>
            </div>
            <p className="text-center text-sm text-muted-foreground">
              Min: 3 bottles • Recommended: 5-10 bottles
            </p>
            <Card className="p-4 bg-muted/50">
              <h4 className="font-medium mb-2">💰 Cost Breakdown</h4>
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <span>{data.bottleQuantity} bottles × ${CUSTOMER_PRICING.BOTTLE_PURCHASE_PRICE}</span>
                  <span>${data.bottleQuantity * CUSTOMER_PRICING.BOTTLE_PURCHASE_PRICE}</span>
                </div>
                <div className="flex justify-between">
                  <span>Deposit × ${CUSTOMER_PRICING.BOTTLE_DEPOSIT}</span>
                  <span>${data.bottleQuantity * CUSTOMER_PRICING.BOTTLE_DEPOSIT}</span>
                </div>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                ℹ️ Deposit is refundable when you return bottles in good condition
              </p>
            </Card>
            <div className="flex gap-3">
              <Button variant="outline" className="flex-1 h-12" onClick={prevStep}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  {[0, 20, 50, 100].map((amount) => (
                    <Button
                      key={amount}
                      variant={data.initialWalletBalance === amount ? 'default' : 'outline'}
                      size="sm"
                      onClick={() => updateData({ initialWalletBalance: amount })}
                    >
                      ${amount}
                    </Button>
                  ))}
